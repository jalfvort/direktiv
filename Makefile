...
docker-ui:
	...
	if [ ! -d ${mkfile_dir_main}/build/docker/ui/web ]; then \
		docker run -v ${mkfile_dir_main}/ui/frontend:/ui chekote/node:14.8.0-alpine /bin/sh -c "mkdir /ui/node_modules; cd /ui && yarn install && NODE_ENV=production yarn build"; \
	fi
