...
docker-ui:
	...
	if [ ! -d ${mkfile_dir_main}/build/docker/ui/web ]; then \
		docker run -v ${mkfile_dir_main}/ui/frontend:/ui -u 1001 -g node chekote/node:14.8.0-alpine /bin/sh -c "id; ls -la /; touch /ui; ls -la /; ls -la /ui; cd /ui && yarn install && NODE_ENV=production yarn build"; \
	fi
